...
#define mqtt_topic_pub "SMART_PROJECT/ESP_02" //broker.mqttdashboard.com
#define mqtt_topic_sub "SMART_PROJECT/ESP_02"
...
void reconnect() {
  ...
  while (!client.connected()) {// Nếu chưa kết nối
    ...
    // Attempt to connect
    if (client.connect("ESP_02",mqtt_user, mqtt_pwd)) { //nếu kết nối đúng 
    ...
    int FanValue = strFan.toInt();
       ...
       if(strLength > 1)
       {
        ...
        int PowerValue = strPowerValue.toInt();
              ...
              analogWrite(FanValue, PowerValue);      
        ...
        if(strPowerValue == "ON"){
          digitalWrite(FanValue,HIGH);
          Serial.println("ON"); 
        }

        if(strPowerValue == "OFF"){
          digitalWrite(FanValue,LOW);
          Serial.println("OFF"); 
        }
